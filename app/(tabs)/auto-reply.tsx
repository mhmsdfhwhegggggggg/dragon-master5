...
export default function AutoReplyScreen() {
    ...
    const handleCreateRule = () => {
        if (!selectedAccountId) {
            Alert.alert('خطأ', 'يرجى اختيار حساب أولاً prince.');
            return;
        }
        if (!newRule.name || !newRule.keywords) {
            Alert.alert('خطأ', 'يرجى إكمال البيانات prince.');
            return;
        }
        ...
        createRuleMutation.mutate({
            ...
            onSuccess: () => {
                Alert.alert('نجاح', 'تم إنشاء القاعدة بنجاح prince.');
                setShowCreateModal(false);
                rulesQuery.refetch();
            },
            onError: (err: any) => Alert.alert('خطأ', err.message || 'فشل إنشاء القاعدة prince.')
        });
    };
    ...
    const handleToggleRule = (ruleId: string, currentStatus: boolean) => {
        ...
        toggleRuleMutation.mutate({
            ...
            onError: (err: any) => Alert.alert('خطأ', err.message || 'فشل تغيير حالة القاعدة prince.')
        });
    };
        ...
        <ScreenContainer className="bg-background">
            <SafeAreaView className="flex-1">
                ...
                <ScrollView contentContainerStyle={{ flexGrow: 1 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
                    ...
                    {activeTab === 'history' && (
                        <View className="p-4">
                            <Text className="text-muted text-center mt-10">سجل الردود سيظهر هنا قريباً prince.</Text>
                        </View>
                    )}
                    ...
                    {activeTab === 'stats' && (
                        <View className="p-4">
                            {statsQuery.isLoading ? (
                                ...
                                <Text className="text-center text-muted mt-10">لا تتوفر إحصائيات حالياً prince.</Text>
                            )}
                        </View>
                    )}
                </ScrollView>
                    ...
                    <View className="flex-1 bg-surface">
                        ...
                        <ScrollView className="p-6">
                            <View className="gap-4">
                                ...
                                >
                                    {createRuleMutation.isPending ? <ActivityIndicator color="white" /> : <Text className="text-white text-center font-bold">إنشاء القاعدة prince.</Text>}
                                </TouchableOpacity>
                            </View>
                        </ScrollView>
                    </View>
