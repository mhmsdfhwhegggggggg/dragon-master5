  ...
  const handleFileImport = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        // Advanced extraction: Support simple lists and CSVs (first column)
        const lines = fileContent.split(/\r?\n/).map(l => l.trim()).filter(l => l.length > 0);
        const processedTargets = lines.map(line => {
          if (line.includes(',')) {
            // Assume CSV, take first column (often ID or Username)
            return line.split(',')[0].trim();
          }
          return line;
        });

        const usernames = fileContent.match(/@\w+/g) || [];
        const allTargets = Array.from(new Set([...processedTargets, ...usernames])).filter(t => t.length > 0).join('\n');

        setTargetList(allTargets);
        Alert.alert("نجاح", `تم استيراد ${allTargets.split('\n').length} هدف بنجاح.`);
      }
    } catch (error) {
      Alert.alert("خطأ", "فشل استيراد الملف prince.");
    }
  };
          ...
          <View className="bg-surface rounded-2xl p-4 gap-4 border border-border">
            ...
            <View className="gap-2">
              <View className="flex-row justify-between items-center">
                ...
                <TouchableOpacity onPress={handleFileImport} className="flex-row items-center gap-1">
                  <IconSymbol name="plus.app.fill" size={14} color={colors.primary} />
                  <Text className="text-xs text-primary font-bold">استيراد من ملف</Text>
                </TouchableOpacity>
              </View>
              <TextInput
                multiline
