...
export default function ExtractionScreen() {
  ...
  const handleExport = async (format: 'txt' | 'csv') => {
    if (!groupId) return Alert.alert("تنبيه", "يرجى إدخال معرّف الجروب أولاً");
    ...
    exportMutation.mutate({
      ...
      onSuccess: async (data: any) => {
        ...
        await FileSystem.writeAsStringAsync(fileUri, data.content, { encoding: FileSystem.EncodingType.UTF8 });

        if (await Sharing.isAvailableAsync()) {
          await Sharing.shareAsync(fileUri);
        } else {
          Alert.alert("نجاح", `تم حفظ الملف: ${data.fileName}`);
        }
      },
      onError: (error: any) => {
        Alert.alert("خطأ", error.message || "فشل تصدير الأعضاء");
      }
    });
  };
            ...
            <View className="gap-3">
              ...
              <View className="flex-row gap-3">
                <TouchableOpacity
                  ...
                  className="flex-1 bg-surface border border-border p-4 rounded-xl items-center flex-row justify-center gap-2"
                >
                  <IconSymbol name="doc.text.fill" size={18} color={colors.muted} />
                  <Text className="text-foreground font-bold">تصدير TXT</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  ...
                  className="flex-1 bg-surface border border-border p-4 rounded-xl items-center flex-row justify-center gap-2"
                >
                  <IconSymbol name="tablecells.fill" size={18} color={colors.muted} />
                  <Text className="text-foreground font-bold">تصدير CSV</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
