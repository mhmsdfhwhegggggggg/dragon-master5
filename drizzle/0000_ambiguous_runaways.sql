...
CREATE TABLE `app_permissions` (
	...
	`permission_type` text DEFAULT 'trial' NOT NULL,
	`max_accounts` integer DEFAULT 1,
	`max_messages_per_day` integer DEFAULT 100,
	`max_operations_per_day` integer DEFAULT 10,
	`features` text,
	`created_at` integer,
...
--> statement-breakpoint
CREATE UNIQUE INDEX `app_permissions_device_id_unique` ON `app_permissions` (`device_id`);--> statement-breakpoint
CREATE UNIQUE INDEX `app_permissions_permission_key_unique` ON `app_permissions` (`permission_key`);--> statement-breakpoint
