...
export async function healthCheck(req: Request, res: Response) {
  ...
  if (memUsagePercent > 90) {
    ...
    // Memory warning alone does NOT degrade status - server is still functional
  }

  // Return 503 only if database (critical dependency) is down
  const hasDBFailure = health.checks.database === "error";
  const statusCode = hasDBFailure ? 503 : 200;
