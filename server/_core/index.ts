...
async function startServer() {
  ...
  app.use((req, res, next) => {
    ...
    if (origin) {
      ...
      // In production, allow requests from mobile apps (which may have empty origin)
  ...
  // 404 Handler - MUST return JSON for API calls
  app.use((req, res) => {
    res.status(404).json({
      error: "Not Found",
      message: `Path ${req.originalUrl} not found on FALCON server.`,
      code: "NOT_FOUND"
    });
  });
  ...
  server.listen(port, "0.0.0.0", () => {
    console.log(`[api] server listening on port ${port} (strictly enforced)`);
