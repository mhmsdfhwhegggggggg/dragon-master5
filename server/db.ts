  ...
  if (!_db && url) {
    try {
      ...
      if (error.code === '28P01') {
        ...
        console.error("[Database] CONNECTION REFUSED: Ensure your DB host is reachable and not blocked by a firewall.");
      } else {
        console.error("[Database] FATAL CONNECTION ERROR:", error.message || error);
        console.error("[Database] Provided URL type:", url.split(':')[0]);
      }
      _db = null;
      _client = null;
    }
...
// Create a proxy for backward compatibility and convenience
export const db = new Proxy({}, {
  get(target, prop) {
    if (!_db) {
      throw new Error('Database not initialized. Please ensure getDb() has been called and awaited.');
    }
...
export const client = new Proxy(() => { }, {
  apply(target, thisArg, argArray) {
    if (!_client) {
      throw new Error('Database client not initialized. Please ensure getDb() has been called and awaited.');
    }
    return (_client as any)(...argArray);
  },
  get(target, prop) {
    if (!_client) {
      throw new Error('Database client not initialized. Please ensure getDb() has been called and awaited.');
...
export async function getOperationsCountToday(): Promise<number> {
  const database = (await getDb()) as any;
  if (!database) return 0;
  const result = await database.execute(sql`SELECT count(*) as count FROM activity_logs WHERE created_at >= ${new Date(new Date().setHours(0, 0, 0, 0))}`);
  return Number(result[0]?.count) || 0;
}
