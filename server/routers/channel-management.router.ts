...
import { channelShield } from "../services/channel-shield";
import { JobQueue } from "../_core/job-queue";
...
export const channelManagementRouter = router({
  ...
  scheduleContent: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const job = await JobQueue.getInstance().addJob("post-content", {
          accountId: input.accountId,
  ...
  /**
   * Get channel shield settings
   */
  getShieldSettings: protectedProcedure
    .input(z.object({
      accountId: z.number(),
      channelId: z.string()
    }))
    .query(async ({ input }) => {
      try {
        const settings = await channelShield.getShieldSettings(input.channelId);
        return {
          success: true,
          data: settings
        };
  ...
  /**
   * Update channel shield settings
   */
  updateShieldSettings: protectedProcedure
    .input(z.object({
      accountId: z.number(),
      channelId: z.string(),
      settings: z.object({
        isProtected: z.boolean().optional(),
        maxAddsPerDay: z.number().optional(),
        sensitivityLevel: z.enum(['safe', 'normal', 'strict']).optional(),
        autoCooldown: z.boolean().optional(),
    ...
    .mutation(async ({ input }) => {
      try {
        const settings = await channelShield.updateShieldSettings(input.channelId, input.settings);
        return {
          success: true,
          data: settings,
