    ...
    try {
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        for (const targetChannelId of options.targetChannelIds) {
          try {
            // 1. Anti-Ban Integration prince
            const recommendation = await this.antiBan.analyzeOperation({
              ...
              accountAge: 180, // Default to safe age if not known
              recentActivityCount: 0, // Should be fetched from activity logs
              proxyUsed: false
            });

            if (recommendation.action === 'stop_operation' || recommendation.action === 'emergency_shutdown') {
              this.logger.warn('[Channel] Transfer halted by Anti-Ban engine', { reason: recommendation.reason });
              break;
            }

            // Calculate delay
            const baseDelay = this.calculateTransferDelay(options.schedule, i);
            const totalDelay = Math.max(baseDelay, recommendation.delay);
  ...
  private async applyModifications(
    ...
    if (message.media) {
      try {
        ...
        // GHOST MODE: Download and re-upload to remove "Forwarded" tag
        ...
        if (buffer) {
          ...
          if (message.photo) {
            contentType = 'image';
            mediaPath = path.join(mediaDir, `ghost_photo_${timestamp}.jpg`);
            fs.writeFileSync(mediaPath, buffer as Buffer);
          } else if (message.video) {
            contentType = 'video';
            mediaPath = path.join(mediaDir, `ghost_video_${timestamp}.mp4`);
            fs.writeFileSync(mediaPath, buffer as Buffer);
          } else if (message.document) {
            contentType = 'file';
            let filename = `ghost_file_${timestamp}`;
            const specifiedName = (message.media as any).document?.attributes?.find((a: any) => a.className === 'DocumentAttributeFilename')?.fileName;
            if (specifiedName) filename = `ghost_${specifiedName}`;
        ...
        this.logger.error('[Channel] Ghost Transfer failed, defaulting to text-only', { error });
      }
    }
