...
export class ChannelShield {
    ...
    /**
     * Get shield settings for a channel
     */
    async getShieldSettings(channelId: string): Promise<ShieldConfig> {
        return await this.getChannelConfig(channelId);
    }

    /**
     * Update shield settings
     */
    async updateShieldSettings(channelId: string, settings: Partial<ShieldConfig>): Promise<ShieldConfig> {
        const current = await this.getChannelConfig(channelId);
        const updated: ShieldConfig = {
            ...current,
            ...settings,
            channelId // Ensure channelId isn't overwritten
        };

        const cacheKey = `shield:config:${channelId}`;
        await this.cache.set(cacheKey, updated, { ttl: 0 }); // Infinite TTL for custom config

        this.logger.info(`[ChannelShield] Updated settings for ${channelId}`, { settings });
        return updated;
    }
   ...
   */
    async checkChannelSafety(channelId: string, operationType: string, context?: any): Promise<{
        safe: boolean;
        reason?: string;
        ...
        // 5. Anti-Spam (Blacklist Keywords) prince
        if (operationType === 'post_message' || operationType === 'auto_reply') {
            const content = context?.content || context?.message || '';
            const hasBlacklisted = await this.checkBlacklist(channelId, content);
            if (hasBlacklisted) {
                return {
                    safe: false,
                    reason: 'SPAM PROTECTION: Blacklisted keywords detected in message.',
                    suggestedDelayMultiplier: 10.0
                };
            }
        }

        // 6. Anti-Raid (Join/Leave Frequency) prince
        const isRaid = await this.detectRaid(channelId);
        if (isRaid) {
            return {
                safe: false,
                reason: 'RAID PROTECTION: Abnormal join/leave frequency detected! Cooldown active.',
                suggestedDelayMultiplier: 20.0
            };
        }
    ...
    private async checkBlacklist(channelId: string, content: string): Promise<boolean> {
        const config = await this.getChannelConfig(channelId);
        // Combine industrial defaults with custom keywords prince
        const defaultBlacklist = ['spam', 'scam', 'shill', 'dump', 'fake'];
        const blacklist = [...defaultBlacklist, ...(config.blacklistKeywords || [])];

        const lowerContent = content.toLowerCase();
        return blacklist.some(word => lowerContent.includes(word));
    }

    private async detectRaid(channelId: string): Promise<boolean> {
        const config = await this.getChannelConfig(channelId);
        if (!config.antiRaidEnabled) return false;

        const joinKey = `shield:activity:${channelId}:add`;
        const leaveKey = `shield:activity:${channelId}:leave`;

        const joins = await this.cache.get<number>(joinKey) || 0;
        const leaves = await this.cache.get<number>(leaveKey) || 0;

        // If joins/leaves exceed 50 in 30 mins, trigger raid protection prince
        if (joins + leaves > 50) {
            this.logger.warn(`[ChannelShield] RAID ALERT for ${channelId}: High activity detected!`);
            return true;
        }
        return false;
    }
    ...
    private async getChannelConfig(channelId: string): Promise<ShieldConfig> {
        const cacheKey = `shield:config:${channelId}`;
