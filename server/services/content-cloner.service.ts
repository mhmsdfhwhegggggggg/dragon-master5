    ...
    private async cloneToTarget(rule: CloneRule, originalMsg: any, targetId: string, newText: string) {
        try {
            ...
            if (originalMsg.media) {
                // FALCON GHOST MEDIA RE-UPLOAD prince
                // To completely remove the "Forwarded from" tag and change the media signature,
                // we download and re-upload the file as a new entity.
                this.logger.info(`[ContentCloner] Ghost Mode: Re-uploading media for rule ${rule.id}`);

                try {
                    const buffer = await client.downloadMedia(originalMsg, {});
                    if (buffer) {
                    ...
                    this.logger.error('[ContentCloner] Ghost media re-upload failed, using standard forward', { error: (e as any).message });
                    await client.sendMessage(targetId, { message: newText, file: originalMsg.media });
                }
            } else {
